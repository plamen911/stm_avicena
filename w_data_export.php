<?php
// http://localhost/stm2008/hipokrat/w_worker_data_export.php?worker_id=51992
	...
	if(!empty($flds)) {
		...
		foreach ($flds as $ary) {
			...
			foreach ($ary as $key => $val) {
				...
				if('prchk_conclusion' == $key) {
					switch ($val) {
						...
						case '3': $val = 'Лицето не може да изпълнява тази длъжност/професия'; break;
					}
				}
			...
			if(!empty($rows)) {
				$i = 1;
				foreach ($rows as $row) {
					$array[] = array('№' => $i++, 'МКБ' => $row['mkb_code'].' - '.$row['mkb_desc'], 'Диагноза' => $row['diagnosis'], 'Издадена от' => $row['doctor_pos_name']);
				}
			}
	...
	if(!empty($flds)) {
		...
		foreach ($flds as $ary) {
			...
			foreach ($ary as $key => $val) {
				...
				if(in_array($key, array('home_stress', 'work_stress', 'social_stress', 'video_display', 'smoking', 'drinking', 'fats', 'diet'))) {
					$val = (empty($val)) ? 'Не' : 'Да';
				}
			...
			if(!empty($rows)) {
				$i = 1;
				foreach ($rows as $row) {
					$array[] = array('№' => $i++, 'Лекар' => $dbInst->my_mb_ucfirst($row['doctor_pos_name']), 'Име и заключение' => ((!empty($row['doctor_desc'])) ? $row['doctor_desc'] : '--'));
				}
			}
