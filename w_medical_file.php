...
<ol style='margin-top:0cm' start=1 type=1>
 <li class=MsoNormal style='mso-list:l0 level1 lfo1;tab-stops:list 36.0pt'><b>Настоящ
     – </b><span style='mso-bidi-font-weight:bold'><?=mb_strtoupper(HTMLFormat($f['position_name']), 'utf-8')?> – от
     <?=(($f['date_curr_position_start2']!='')?$f['date_curr_position_start2'].' г.':'')?><o:p></o:p></span></li>
...
?>
<?php
$rows = $dbInst->getPrchkDiagnosis($worker_id);
if($rows) {
?>
...
if($rows) {
	$_arr = array();
	foreach ($rows as $row) {
		if($row['doctor_desc'] == '') continue;
		$_arr[] = $dbInst->my_mb_ucfirst(HTMLFormat($row['doctor_pos_name']));
	}
	if(count($_arr)) {
		echo ' – '.implode(', ', $_arr);
	}
...
<p class=MsoNormal>Фамилна обремененост</p>
...
if($rows) {
	foreach ($rows as $row) {
		if($row['doctor_desc'] == '') continue;
?>
<p class=MsoNormal><b><?=$dbInst->my_mb_ucfirst(HTMLFormat($row['doctor_pos_name']))?></b>: <?=HTMLFormat($row['doctor_desc'])?></p>
<?php
	}
}
?>
