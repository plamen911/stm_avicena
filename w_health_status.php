...
function getMedicalCheckups($worker_id, $chkY) {
	...
	if(!empty($fields)) {
		foreach ($fields as $field) {
			...
			if(!empty($flds)) {
				foreach ($flds as $f) {
					if(!empty($f['doctor_desc'])) {
						$doctor_pos_name = $dbInst->my_mb_ucfirst(HTMLFormat($f['doctor_pos_name']));
						...
						$out .= '<p class=MsoNormal><span style=\'font-size:10.0pt\'>'.$i++.'. '.$doctor_pos_name.'- '.HTMLFormat($f['doctor_desc']).'<o:p></o:p></span></p>';
					}
				}
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0 width="100%"
 ...
 <tr style='mso-yfti-irow:<?=($i+1)?>;mso-yfti-lastrow:yes'>
  ...
  echo (!empty($row['prchk_date'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>'.date('d.m.Y', strtotime($row['prchk_date'])).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_obstetrician'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Акушер-г.: '.HTMLFormat($row['prchk_obstetrician']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_dermatologist'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Дерм.: '.HTMLFormat($row['prchk_dermatologist']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_internal_diseases'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Инт.: '.HTMLFormat($row['prchk_internal_diseases']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_neurologist'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Невр.: '.HTMLFormat($row['prchk_neurologist']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_GP'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>ОПЛ: '.HTMLFormat($row['prchk_GP']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_ophthalmologist'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Офт.: '.HTMLFormat($row['prchk_ophthalmologist']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_pathologist'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Профп.: '.HTMLFormat($row['prchk_pathologist']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_dentist'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>Стом.: '.HTMLFormat($row['prchk_dentist']).'<o:p></o:p></span></p>' : '';
  echo (!empty($row['prchk_UNG'])) ? '<p class=MsoNormal><span style=\'font-size:10.0pt\'>УНГ: '.HTMLFormat($row['prchk_UNG']).'<o:p></o:p></span></p>' : '';
