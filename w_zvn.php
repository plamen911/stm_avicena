...
/*$query = "	SELECT w.worker_id,
			...
			AND (d.medical_types LIKE '%\"1\"%' OR d.medical_types LIKE '%\"2\"%')
...
*/
$query = "	SELECT w.worker_id AS worker_id,
			m.group_id AS group_id,
			g.group_name AS group_name, 
			d.reason_id AS reason_id,
			i.position_name AS position_name,
			i.position_id AS position_id,
			COUNT(i.position_id) AS num_employees,
			SUM(d.days_off) AS num_days_off
			FROM patient_charts d
			LEFT JOIN mkb m ON (m.mkb_id = d.mkb_id)
  ...
  style='font-size:10.0pt'>над 10 г.</span></b></p>
   ...
   if(!empty($rows)) {
   	...
   	foreach ($rows as $row) {
   		...
   		if(empty($row['position_name'])) continue;

   		$zvn_employees[$row['worker_id']] = $row['worker_id'];

   		if($reason_id != $row['reason_id']) {
   			$reason_name = $row['reason_id'];
   		...
   		if($group_id != $row['group_id']) {
   			$group_name = substr(strrchr($row['group_name'], '('), 1, -1);
   		...
   		$position_id = (!empty($row['position_id'])) ? $row['position_id'] : 0;
   		$position_name = (!empty($row['position_name'])) ? $row['position_name'] : '';
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0 width="100%"
 ...
 if(!empty($rows)) {
 	...
 	foreach ($rows as $row) {
 		...
 		$employees = $dbInst->query("	SELECT i.position_id AS position_id, i.position_name AS position_name
										FROM patient_charts d
										LEFT JOIN mkb m ON (m.mkb_id = d.mkb_id)
										LEFT JOIN workers w ON (w.worker_id = d.worker_id)
										LEFT JOIN firm_struct_map m1 ON (m1.map_id = w.map_id )
 		...
 		$men = $dbInst->query("		SELECT i.position_id AS position_id, i.position_name AS position_name
									FROM patient_charts d
									LEFT JOIN mkb m ON (m.mkb_id = d.mkb_id)
									LEFT JOIN workers w ON (w.worker_id = d.worker_id)
									LEFT JOIN firm_struct_map m1 ON (m1.map_id = w.map_id )
 		...
 		$women = $dbInst->query("	SELECT i.position_id AS position_id, i.position_name AS position_name
									FROM patient_charts d
									LEFT JOIN mkb m ON (m.mkb_id = d.mkb_id)
									LEFT JOIN workers w ON (w.worker_id = d.worker_id)
									LEFT JOIN firm_struct_map m1 ON (m1.map_id = w.map_id )
