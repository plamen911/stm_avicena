...
if(isset($_POST['ajax_action']) && 'download_db' == $_POST['ajax_action']) {
	...
	if(false === $rawdata = $curl->getFile($url)) {
		...
		die('{"errcode":'.$ret['errcode'].',"errmsg":"'.$ret['errmsg'].'"}');
	} else {
		//save db
		$dbdir = 'db/';
		$dbname = basename($url);
...
$(function(){
	$('#lnkSynch').bind('click', function(e){
		e.preventDefault();
		$('#preLoader .preLoaderText p:eq(0)').html('Моля, изчакайте да се свалят данните от сървъра.');
...
echo '<p>&nbsp;</p>';
echo '<p><a href="firms.php">&larr; Обратно</a></p>';
echo '<p>&nbsp;</p>';
include("footer.php");
