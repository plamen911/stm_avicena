<?php
// http://cvs.php.net/viewvc.cgi/pear/Pager/examples/Pager_Wrapper.php?view=markup
// CVS: $Id: Pager_Wrapper.php,v 1.22 2006/03/15 12:29:57 quipo Exp $
//
// Pager_Wrapper
// -------------
//
...
function Pager_Wrapper($query, $pager_options = array(), $disabled = false)
{
	if (!array_key_exists('totalItems', $pager_options)) {
		//  be smart and try to guess the total number of records
		if ($countQuery = rewriteCountQuery($query)) {
			$res = mysql_query($countQuery);
			list($totalItems) = mysql_fetch_array($res);
			if (!$totalItems) {
				return $totalItems;
			}
		} else {
	...
	$res = ($disabled)
	? mysql_query($query." LIMIT 0,$totalItems")
	: mysql_query($query." LIMIT ".($page['from']-1).",".$pager_options['perPage']);
...
function Pager_Wrapper_PDO(&$db, $query, $pager_options = array(), $disabled = false, $fetchMode = PDO::FETCH_BOTH)
{
	/*
	Default fetch style - PDO::FETCH_BOTH (Fetch all of the remaining rows in the result set)
	Array
	(
	[0] => Array
	(
	...
	*/
	...
	$res = ($disabled)
	? $db->prepare($query." LIMIT 0,$totalItems")
	: $db->prepare($query." LIMIT ".($page['from']-1).",".$pager_options['perPage']);
	...
	$res->execute();
	$page['data'] = $res->fetchAll($fetchMode);
	if ($disabled) {
		$page['links'] = '';
		$page['page_numbers'] = array(
		'current' => 1,
