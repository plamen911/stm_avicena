...
class SqliteDB
{
	...
	public function getDoctorsDesc($checkup_id = 0) {
		$sql = sprintf("SELECT c.* , d.doctor_pos_name
						...
						ORDER BY d.`default` DESC, d.`doctor_pos_name`, c.doctor_pos_id", $checkup_id);
		return $this->query($sql);
	}
	...
	{
		$db = $this->getDBHandle();
		$query = "SELECT * FROM lab_indicators ORDER BY indicator_position";
		try {
			$prepstatement = $db->prepare($query);
			if (!$prepstatement) {
				$err = $db->errorInfo();
	...
	public function getWorkersByCharts3($firm_id = 0, $date_from = '2007-01-01 00:00:00', $date_to = '2007-12-31 00:00:00') {
		...
		$sql = "SELECT w.worker_id, w.sex, w.fname, w.sname, w.lname,
				...
				(SELECT COUNT(*) FROM patient_charts WHERE worker_id = w.worker_id
					...
					AND medical_types LIKE '%\"1\"%') AS num_primary
				FROM workers w
				LEFT JOIN firm_struct_map m ON (m.map_id = w.map_id)
				LEFT JOIN firm_positions i ON (i.position_id = m.position_id)
		...
		try {
			...
			switch ($tab) {
					...
					foreach ($aFormValues as $key => $val) {
						if (preg_match('/^indicator_id_(\d+)$/', $key, $matches)) {
							...
							if ($lab_checkup_id) { // Update
								$query = "UPDATE lab_checkups SET indicator_id='$indicator_id', checkup_type='$checkup_type', checkup_level='" .
								floatval($checkup_level) . "' WHERE lab_checkup_id='$lab_checkup_id'";
							} else {
								$query = "INSERT INTO lab_checkups (firm_id, worker_id, checkup_id, indicator_id, checkup_type, checkup_level) VALUES ('$firm_id', '$worker_id', '$checkup_id', '$indicator_id', '$checkup_type', '" .
								floatval($checkup_level) . "')";
							}
							$count = $db->exec($query);
						}
					}
				...
				case 'conclusion':
					...
					// This seems are nowhere used db fields in `medical_checkups` table - see `medical_checkups_doctors`
					$var_list = array('desc_GP' => 'desc_GP', 'desc_pathologist' =>
					'desc_pathologist', 'desc_neurologist' => 'desc_neurologist', 'desc_UNG' =>
					'desc_UNG', 'desc_ophthalmologist' => 'desc_ophthalmologist',
					'desc_dermatologist' => 'desc_dermatologist', 'desc_surgeon' => 'desc_surgeon');
					while (list($var, $param) = @each($var_list)) {
						if (isset($aFormValues[$param]))
						$$var = $this->checkStr($aFormValues[$param]);
					} //end while

					// HACK! - This will not affect the app. logics. In `doctor_desc` are the doctors' name and conclusion.
					// Should be removed in future versions.
					$conclusion = $notes = '';
					$query = sprintf("UPDATE medical_checkups SET conclusion='$conclusion', notes='$notes', date_modified=datetime('now','localtime') WHERE checkup_id = %d",
					$checkup_id);
					//die($query);
					$count = $db->exec($query); //returns affected rows

					foreach ($aFormValues as $key => $val) {
						if (preg_match('/^doctor_desc_(\d+)$/', $key, $matches)) {
							$position = $matches[1];
							$doctor_desc = $this->checkStr($aFormValues['doctor_desc_' . $position]);
							$query = sprintf("UPDATE medical_checkups_doctors SET doctor_desc = '%s' WHERE checkup_id = %d AND position = %d",
							$doctor_desc, $checkup_id, $position);
							$count = $db->exec($query);
						}
					}
					break;
				...
				default:
					...
					'rr_diast' => 'rr_diast', 'hours_activity' => 'hours_activity');
					while (list($var, $param) = @each($var_list)) {
						if (isset($aFormValues[$param]))
						$$var = $this->checkStr($aFormValues[$param]);
					} //end while
					$home_stress = (isset($aFormValues['home_stress'])) ? '1':
					'0';
					$work_stress = (isset($aFormValues['work_stress'])) ? '1':
					'0';
					$social_stress = (isset($aFormValues['social_stress'])) ? '1':
					'0';
					$video_display = (isset($aFormValues['video_display'])) ? '1':
					'0';
					$smoking = (isset($aFormValues['smoking'])) ? '1':
					'0';
					$drinking = (isset($aFormValues['drinking'])) ? '1':
					'0';
					$fats = (isset($aFormValues['fats'])) ? '1':
					'0';
					$diet = (isset($aFormValues['diet'])) ? '1':
					'0';
					...
					$checkup_date = '';
					if (!$checkup_id) {
						$query = "INSERT INTO medical_checkups (firm_id, worker_id, checkup_date, hospital, worker_height, worker_weight, rr_syst, rr_diast, hours_activity, home_stress, work_stress, social_stress, video_display, smoking, drinking, fats, diet, date_added, date_modified) VALUES ($firm_id, $worker_id, '$checkup_date', '$hospital', '$worker_height', '$worker_weight', '$rr_syst', '$rr_diast', '$hours_activity', '$home_stress', '$work_stress', '$social_stress', '$video_display', '$smoking', '$drinking', '$fats', '$diet', datetime('now','localtime'), datetime('now','localtime'))";
						//die($query);
						$count = $db->exec($query); //returns affected rows
						$checkup_id = $db->lastInsertId();

						// Insert doctors
						$query2 = "SELECT * FROM cfg_doctor_positions ORDER BY doctor_pos_id";
						...
						$fields = $prepstatement2->fetchAll();
						$position = 1;
						foreach ($fields as $field) {
							$query = sprintf("INSERT INTO medical_checkups_doctors (checkup_id, doctor_pos_id, position) VALUES (%d, %d, %d)",
							$checkup_id, $field['doctor_pos_id'], $position++);
							$count = $db->exec($query);
						}
					} else {
						$query = "UPDATE medical_checkups SET checkup_date='$checkup_date', hospital='$hospital', worker_height='$worker_height', worker_weight='$worker_weight', rr_syst='$rr_syst', rr_diast='$rr_diast', hours_activity='$hours_activity', home_stress='$home_stress', work_stress='$work_stress', social_stress='$social_stress', video_display='$video_display', smoking='$smoking', drinking='$drinking', fats='$fats', diet='$diet', date_modified=datetime('now','localtime') WHERE checkup_id='$checkup_id'";
						$count = $db->exec($query); //returns affected rows
					}
					break;
			}
	...
	public function processPrchkCheckup($aFormValues, $tab)
	{
		...
		try {
			switch ($tab) {
				case 'specialists':
					$var_list = array('prchk_obstetrician' => 'prchk_obstetrician',
					'prchk_obstetrician_doc' => 'prchk_obstetrician_doc', 'prchk_dermatologist' =>
					'prchk_dermatologist', 'prchk_dermatologist_doc' => 'prchk_dermatologist_doc',
					'prchk_internal_diseases' => 'prchk_internal_diseases',
					'prchk_internal_diseases_doc' => 'prchk_internal_diseases_doc',
					...
					if(!$precheckup_id) {
						$query = "INSERT INTO `medical_precheckups` (`firm_id`, `worker_id`, `prchk_author`, `prchk_date`, `prchk_anamnesis`, `prchk_data`, `prchk_conclusion`, `prchk_conditions`, `prchk_stm_date`, `prchk_obstetrician`, `prchk_obstetrician_doc`, `prchk_dermatologist`, `prchk_dermatologist_doc`, `prchk_internal_diseases`, `prchk_internal_diseases_doc`, `prchk_ophthalmologist`, `prchk_ophthalmologist_doc`, `prchk_pathologist`, `prchk_pathologist_doc`, `prchk_UNG`, `prchk_UNG_doc`, `prchk_neurologist`, `prchk_neurologist_doc`, `prchk_surgeon`, `prchk_surgeon_doc`, `prchk_GP`, `prchk_GP_doc`, `prchk_dentist`, `prchk_dentist_doc`, `prchk_cardiologist`, `prchk_cardiologist_doc`, `prchk_therapeutist`, `prchk_therapeutist_doc`, `prchk_psychiatrist`, `prchk_psychiatrist_doc`, `prchk_radiobiologist`, `prchk_radiobiologist_doc`, `prchk_doctor_tm`, `prchk_doctor_tm_doc`, `prchk_assisant_tm`, `prchk_assisant_tm_doc`, `date_added`, `date_modified`) VALUES ('$firm_id', '$worker_id', '$prchk_author', '$prchk_date', '$prchk_anamnesis', '$prchk_data', '$prchk_conclusion', '$prchk_conditions', '$prchk_stm_date', '$prchk_obstetrician', '$prchk_obstetrician_doc', '$prchk_dermatologist', '$prchk_dermatologist_doc', '$prchk_internal_diseases', '$prchk_internal_diseases_doc', '$prchk_ophthalmologist', '$prchk_ophthalmologist_doc', '$prchk_pathologist', '$prchk_pathologist_doc', '$prchk_UNG', '$prchk_UNG_doc', '$prchk_neurologist', '$prchk_neurologist_doc', '$prchk_surgeon', '$prchk_surgeon_doc', '$prchk_GP', '$prchk_GP_doc', '$prchk_dentist', '$prchk_dentist_doc', '$prchk_cardiologist', '$prchk_cardiologist_doc', '$prchk_therapeutist', '$prchk_therapeutist_doc', '$prchk_psychiatrist', '$prchk_psychiatrist_doc', '$prchk_radiobiologist', '$prchk_radiobiologist_doc', '$prchk_doctor_tm', '$prchk_doctor_tm_doc', '$prchk_assisant_tm', '$prchk_assisant_tm_doc', datetime('now','localtime'), datetime('now','localtime'))";
						$precheckup_id = $this->query($query);
					} else {
						$query = "UPDATE `medical_precheckups` SET prchk_author = '$prchk_author', prchk_date = '$prchk_date', prchk_obstetrician = '$prchk_obstetrician', prchk_obstetrician_doc = '$prchk_obstetrician_doc', prchk_dermatologist = '$prchk_dermatologist', prchk_dermatologist_doc = '$prchk_dermatologist_doc', prchk_internal_diseases = '$prchk_internal_diseases', prchk_internal_diseases_doc = '$prchk_internal_diseases_doc', prchk_ophthalmologist = '$prchk_ophthalmologist', prchk_ophthalmologist_doc = '$prchk_ophthalmologist_doc', prchk_pathologist = '$prchk_pathologist', prchk_pathologist_doc = '$prchk_pathologist_doc', prchk_UNG = '$prchk_UNG', prchk_UNG_doc = '$prchk_UNG_doc', prchk_neurologist = '$prchk_neurologist', prchk_neurologist_doc = '$prchk_neurologist_doc', prchk_surgeon = '$prchk_surgeon', prchk_surgeon_doc = '$prchk_surgeon_doc', prchk_GP = '$prchk_GP', prchk_GP_doc = '$prchk_GP_doc', prchk_dentist = '$prchk_dentist', prchk_dentist_doc = '$prchk_dentist_doc', `prchk_cardiologist` = '$prchk_cardiologist', `prchk_cardiologist_doc` = '$prchk_cardiologist_doc', `prchk_therapeutist` = '$prchk_therapeutist', `prchk_therapeutist_doc` = '$prchk_therapeutist_doc', `prchk_psychiatrist` = '$prchk_psychiatrist', `prchk_psychiatrist_doc` = '$prchk_psychiatrist_doc', `prchk_radiobiologist` = '$prchk_radiobiologist', `prchk_radiobiologist_doc` = '$prchk_radiobiologist_doc', `prchk_doctor_tm` = '$prchk_doctor_tm', `prchk_doctor_tm_doc` = '$prchk_doctor_tm_doc', `prchk_assisant_tm` = '$prchk_assisant_tm', `prchk_assisant_tm_doc` = '$prchk_assisant_tm_doc', date_modified=datetime('now','localtime') WHERE precheckup_id = $precheckup_id";
			...
			$this->processLastPrchkCheckup($worker_id);
		...
		try {
			...
			foreach ($aFormValues as $key => $value) {
				...
				'') {
					...
					$subdivision_name = $this->checkStr($aFormValues['subdivision_name_' . $subdivision_id]);
					if ($subdivision_id) {
		...
		try {
			...
			foreach ($aFormValues as $key => $value) {
				if (preg_match('/wplace_name_(\d+)$/', $key, $matches) && trim($value) != '') {
					$wplace_id = $matches[1];
					$wplace_name = $this->checkStr($aFormValues['wplace_name_' . $wplace_id]);
			...
			foreach ($aFormValues as $key => $value) {
				if (preg_match('/user_name_(\d+)$/', $key, $matches) && trim($value) != '') {
					...
					if ($user_id) {
						...
						$count = $db->exec("INSERT INTO users (user_name, user_pass, user_level, fname, lname, date_created, date_modified, hdd) VALUES ('$user_name', '$user_pass', '3', '$fname', '$lname', datetime('now'), datetime('now'), '" .
						$this->getHDDSerial() . "')");
					}
				}
			}
		...
		try {
			...
			foreach ($aFormValues as $key => $value) {
				if (preg_match('/position_name_(\d+)$/', $key, $matches) && trim($value) != '') {
					...
					$position_name = $this->checkStr($aFormValues['position_name_' . $position_id]);
	...
	public function getChartDaysOff($firm_id = 0, $date_from = '2007-01-01 00:00:00', $date_to = '2007-12-31 00:00:00') {
		...
		$sql = "SELECT t.*
				...
				AND ( w.date_retired = '' OR julianday(w.date_retired) >= julianday('$date_from') )
				AND ( w.date_curr_position_start = '' OR julianday(w.date_curr_position_start) <= julianday('$date_to') )
				AND ( t.medical_types LIKE '%\"1\"%' OR t.medical_types LIKE '%\"2\"%' )
				AND
				(
					(julianday(t.hospital_date_from) >= julianday('$date_from')) AND (julianday(t.hospital_date_from) <= julianday('$date_to'))
				)
	...
	*/
     ...
     * @param $worker_id - worker_id
     */
	public function getPrchkDocDiagnosis($worker_id) {
		$data = array();
		$query = sprintf("SELECT * FROM workers WHERE worker_id = %d AND is_active = '1' AND date_retired = ''",
		$worker_id);
		if ($row = $this->fnSelectSingleRow($query)) {
			if ($row['prchk_obstetrician'] != '' || $row['prchk_obstetrician_doc'] != '') {
				$tmp['doctor_pos_name'] = 'Акушер-гинеколог';
				$tmp['doc_name'] = $row['prchk_obstetrician_doc'];
				$tmp['doc_conclusion'] = $row['prchk_obstetrician'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 1);
				$data[] = $tmp;
			}
			if ($row['prchk_dermatologist'] != '' || $row['prchk_dermatologist_doc'] != '') {
				$tmp['doctor_pos_name'] = 'Дерматовенеролог';
				$tmp['doc_name'] = $row['prchk_dermatologist_doc'];
				$tmp['doc_conclusion'] = $row['prchk_dermatologist'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 2);
				$data[] = $tmp;
			}
			if ($row['prchk_internal_diseases'] != '' || $row['prchk_internal_diseases_doc'] !=
			'') {
				$tmp['doctor_pos_name'] = 'Интернист';
				$tmp['doc_name'] = $row['prchk_internal_diseases_doc'];
				$tmp['doc_conclusion'] = $row['prchk_internal_diseases'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 3);
				$data[] = $tmp;
			}
			if ($row['prchk_neurologist'] != '' || $row['prchk_neurologist_doc'] != '') {
				$tmp['doctor_pos_name'] = 'Невролог';
				$tmp['doc_name'] = $row['prchk_neurologist_doc'];
				$tmp['doc_conclusion'] = $row['prchk_neurologist'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 4);
				$data[] = $tmp;
			}
			if ($row['prchk_GP'] != '' || $row['prchk_GP_doc'] != '') {
				$tmp['doctor_pos_name'] = 'ОПЛ (общо практикуващ лекар)';
				$tmp['doc_name'] = $row['prchk_GP_doc'];
				$tmp['doc_conclusion'] = $row['prchk_GP'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 5);
				$data[] = $tmp;
			}
			if ($row['prchk_ophthalmologist'] != '' || $row['prchk_ophthalmologist_doc'] !=
			'') {
				$tmp['doctor_pos_name'] = 'Офталмолог';
				$tmp['doc_name'] = $row['prchk_ophthalmologist_doc'];
				$tmp['doc_conclusion'] = $row['prchk_ophthalmologist'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 6);
				$data[] = $tmp;
			}
			if ($row['prchk_pathologist'] != '' || $row['prchk_pathologist_doc'] != '') {
				$tmp['doctor_pos_name'] = 'Профпатолог';
				$tmp['doc_name'] = $row['prchk_pathologist_doc'];
				$tmp['doc_conclusion'] = $row['prchk_pathologist'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 7);
				$data[] = $tmp;
			}
			if ($row['prchk_dentist'] != '' || $row['prchk_dentist_doc'] != '') {
				$tmp['doctor_pos_name'] = 'Стоматолог';
				$tmp['doc_name'] = $row['prchk_dentist_doc'];
				$tmp['doc_conclusion'] = $row['prchk_dentist'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 8);
				$data[] = $tmp;
			}
			if ($row['prchk_UNG'] != '' || $row['prchk_UNG_doc'] != '') {
				$tmp['doctor_pos_name'] = 'УНГ';
				$tmp['doc_name'] = $row['prchk_UNG_doc'];
				$tmp['doc_conclusion'] = $row['prchk_UNG'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 9);
				$data[] = $tmp;
			}
			if ($row['prchk_surgeon'] != '' || $row['prchk_surgeon_doc'] != '') {
				$tmp['doctor_pos_name'] = 'Хирург';
				$tmp['doc_name'] = $row['prchk_surgeon_doc'];
				$tmp['doc_conclusion'] = $row['prchk_surgeon'];
				$tmp['diagnosis'] = $this->getPrchkDocDiagnosisN($worker_id, 10);
				$data[] = $tmp;
			}
		}
		return $data;
	...
	public function getPrchkDocDiagnosisN($worker_id, $doctor_pos_id = 1)
	{
		$precheckup_id = $this->GiveValue('precheckup_id', 'medical_precheckups', "WHERE `worker_id` = $worker_id ORDER BY `prchk_date` DESC, `precheckup_id` DESC LIMIT 1", 0);
		$query = sprintf("	SELECT d.*,
							m.mkb_desc, m.mkb_code,
							p.doctor_pos_name
							FROM prchk_diagnosis d
							...
							LEFT JOIN cfg_doctor_positions p ON (p.doctor_pos_id = d.published_by)
							WHERE d.precheckup_id = %d
							AND d.published_by = %d
							ORDER BY d.prchk_id", $precheckup_id, $doctor_pos_id);
		$arr = array();
		if ($fields = $this->fnSelectRows($query)) {
			foreach ($fields as $field) {
				$arr[] = $field['mkb_id'];
			}
		}
